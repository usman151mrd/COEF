...
if(isset($_GET['lid']))
{
	...
	if($count==0)
	{
		$sql = "INSERT INTO `related_topic_status`(`t_id`, `user_id`, `status`) VALUES($lid,$user_id,'like')";
		$mydb->setQuery($sql);
		$mydb->insert();
	...
	{
		$sql = "UPDATE `related_topic_status` SET `status`='like' WHERE `user_id`=$user_id AND `t_id`=$lid";
		$mydb->setQuery($sql);
		$mydb->update();
